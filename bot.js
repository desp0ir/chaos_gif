...
client.gif = {
  kategoriler: ["770558896690430003"], //gif kategori idleri
  log: "807933369979109377", //Gif-Log (kanal id)
  sunucu: "Chaos", //Sunucunuzun İsmi
  rastgele: {
    PP: "807933289880617001", //Random PP (kanal id)
    GIF: "807933317025366016" //Random Gif (kanal id)
  }
...
if(gif > 0 && pp > 0){
  ...
  client.channels.cache.get(client.gif.log).send(new Discord.MessageEmbed().setColor('RANDOM').setAuthor(' 🔥').setDescription(`${emojis[random]} \`•\` **${msg.author.tag}** (\`${msg.author.id}\`) kişisi,\n<#${msg.channel.id}> kanalına ${mesaj} gönderdi.\nBu kişi şuanda kanallara toplam ${db.fetch(`sayı.${msg.author.id}`)||0} pp/gif göndermiş.`))
}// 
})
...
console.log('Bot Başarıyla Aktif Edildi by Lilith ')
